...
function obtenerPrefijoApiSpaceInvaders() {
    ...
    if (typeof backendUrl === 'string' && backendUrl.trim()) {
        return backendUrl.trim().replace(/\/$/, ''); // quita trailing slash
    }

    // Por defecto, asume que el backend está en localhost:8083 (Docker)
    // En producción, cambiar a la URL real del backend
    if (typeof window.location !== 'undefined' && window.location.hostname) {
        const hostname = window.location.hostname;
        const port = hostname === 'localhost' || hostname === '127.0.0.1' ? 8083 : window.location.port;
...
function obtenerSalaId() {
    const params = new URLSearchParams(window.location.search || '');
    const backendSalaId = params.get('backendSalaId');
    if (typeof backendSalaId === 'string' && backendSalaId.trim()) {
        return backendSalaId.trim();
    }
...
function obtenerNombreJugador() {
    const params = new URLSearchParams(window.location.search);
    const nombreEnQuery = params.get('player') ?? params.get('nombreJugador');
...
function obtenerJugadorIdPersistente() {
    const params = new URLSearchParams(window.location.search);
    const jugadorIdEnQuery = params.get('jugadorId') ?? params.get('playerId');
    if (typeof jugadorIdEnQuery === 'string' && jugadorIdEnQuery.trim()) {
        return jugadorIdEnQuery.trim();
    }
...
function initGame() {
    ...
    if (!canalComunicacion) {
        ...
        const conexion = new FetchApiConexion(
            `${apiBase}/api/salas/${salaSegmento}/space-invaders/eventos`,
            `${apiBase}/api/salas/${salaSegmento}/space-invaders/actualizaciones`,
            {
            playerId,
