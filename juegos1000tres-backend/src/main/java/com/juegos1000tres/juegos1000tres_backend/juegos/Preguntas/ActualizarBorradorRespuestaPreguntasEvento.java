...
public class ActualizarBorradorRespuestaPreguntasEvento implements Evento<String> {

    private final PreguntasJuego juego;

    public ActualizarBorradorRespuestaPreguntasEvento(PreguntasJuego juego) {
        this.juego = Objects.requireNonNull(juego, "El juego es obligatorio");
    }

    @Override
    public void hacer(String payload, ContextoEvento contexto) {
        this.juego.actualizarBorradorDesdePayload(payload, contexto);
    }
}
