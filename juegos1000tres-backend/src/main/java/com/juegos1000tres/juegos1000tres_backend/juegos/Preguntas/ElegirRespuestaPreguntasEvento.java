...
public class ElegirRespuestaPreguntasEvento implements Evento<String> {

    private final PreguntasJuego juego;

    public ElegirRespuestaPreguntasEvento(PreguntasJuego juego) {
        this.juego = Objects.requireNonNull(juego, "El juego es obligatorio");
    }

    @Override
    public void hacer(String payload, ContextoEvento contexto) {
        this.juego.elegirRespuestaDesdePayload(payload, contexto);
    }
}
