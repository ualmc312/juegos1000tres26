...
public class TapTapRegistrarPuntoEvento implements Evento<String> {

    private final TapTapService tapTapService;
    private final String salaId;

    public TapTapRegistrarPuntoEvento(TapTapService tapTapService, String salaId) {
    ...
    public void hacer(String payload, ContextoEvento contexto) {
        ...
        try {
            String jugadorId = extractField(payload, "jugadorId");
            if (jugadorId != null && !jugadorId.isBlank()) {
                this.tapTapService.registrarPunto(this.salaId, jugadorId);
            }
            ...
            System.err.println("Error al registrar punto en TapTap: " + e.getMessage());
        }
    }
