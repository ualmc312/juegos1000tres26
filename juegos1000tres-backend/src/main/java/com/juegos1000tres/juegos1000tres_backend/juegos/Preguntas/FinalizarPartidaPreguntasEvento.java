...
public class FinalizarPartidaPreguntasEvento implements Evento<String> {

    private final PreguntasJuego juego;

    public FinalizarPartidaPreguntasEvento(PreguntasJuego juego) {
        this.juego = Objects.requireNonNull(juego, "El juego es obligatorio");
    }

    @Override
    public void hacer(String payload, ContextoEvento contexto) {
        this.juego.finalizarPartidaDesdePayload(payload, contexto);
    }
}